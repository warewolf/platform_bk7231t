...
void bk_cw_pwm_set_duty_cycle(uint8 channel_num, uint32 duty_cycle)
{
    ...
    TPWM_PRT("bk_cw_pwm_set_duty_cycle: %d %ld\r\n", channel_num, duty_cycle);
    ...
    if (PWM_SUCCESS != ret)
    {
        TPWM_FATAL("bk_cw_pwm_set_duty_cycle: error param!\r\n");
    }
    ASSERT(PWM_SUCCESS == ret);
}
...
void bk_cw_pwm_set_end_value(uint8 channel_num, uint32 end_value)
{
    ...
    TPWM_PRT("bk_cw_pwm_set_end_value: %d %ld\r\n", channel_num, end_value);
    ...
    if (PWM_SUCCESS != ret)
    {
        TPWM_FATAL("bk_cw_pwm_set_end_value: error param!\r\n");
    }
    ASSERT(PWM_SUCCESS == ret);
}
