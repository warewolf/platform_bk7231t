...
static int pwm_dev_control(tuya_pwm_t *pwm, uint8_t cmd, void *arg)
{
    ...
    switch (cmd) {
        ...
        case TUYA_PWM_PULSE_SET_CMD:
            // pwm极性进行了设置，但是设置不正常
            if((PWM_POLARITTY_INIT_CNT != polarity_init_cnt) &&(polarity_init_cnt != 0)) { // bk7231t上只能支持一对pwm互斥极性
                bk_printf("set pwm cw mutex, must set in pairs!!!\r\n");
                return OPRT_INVALID_PARM;
            }
            ...
            // 正确设置了pwm 互斥极性
            if(((channel_num == polarity_channel[0]) || (channel_num == polarity_channel[1])) && (PWM_POLARITTY_INIT_CNT == polarity_init_cnt )) {
                if(channel_num == polarity_channel[0]){ // 互斥极性第一路pwm
                    polarity_duty1 = duty;
                    
                } else if(channel_num == polarity_channel[1]){
                    bk_printf("cw duty1 %d duty2 %d\r\n", polarity_duty1, duty);
                    ret = bk_cw_pwm_reset_duty_cycle(polarity_channel[0], polarity_channel[1], polarity_duty1, duty, period, 0);
                } else {
                    return OPRT_INVALID_PARM;
                }
            }
           
            ret = bk_pwm_update_param(channel_num, period, duty);
