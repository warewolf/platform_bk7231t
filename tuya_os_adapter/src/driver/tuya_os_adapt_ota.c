...
{    
    ...
    if(ug_proc == NULL) {
        bk_printf("ota don't start or start err, can't end inform!\r\n");
        return OPRT_OS_ADAPTER_INVALID_PARM;
    }
    ...
    if(flash_checksum != ug_proc->file_header.bin_sum) {
        bk_printf("verify_ota_checksum err  checksum(0x%x)  file_header.bin_sum(0x%x)\r\n",flash_checksum,ug_proc->file_header.bin_sum);
    ...
    bk_printf("the gateway upgrade success\r\n");
