                                   ...
                                   const unsigned char *data, const unsigned int len, unsigned int *remain_len, void *pri_data)
{
    ...
    switch (ug_proc->stat) {
        case UGS_RECV_HEADER: {
            ...
            LOG_NOTICE("header_flag(0x%x) tail_flag(0x%x) head_sum(0x%x-0x%x) bin_sum(0x%x)\r\n", ug_proc->file_header.header_flag,
                       ug_proc->file_header.tail_flag, ug_proc->file_header.head_sum, sum_tmp, ug_proc->file_header.bin_sum);

            if ((ug_proc->file_header.header_flag !=  UG_PKG_HEAD) || (ug_proc->file_header.tail_flag !=  UG_PKG_TAIL) ||
                    (ug_proc->file_header.head_sum != sum_tmp)) {
            ...
            LOG_ERR("get right bin_file_header!!!\r\n");
        ...
        case UGS_RECV_IMG_DATA: {    //dont have set lock for flash!
            *remain_len = len;
            ...
            if ((ug_proc->recv_data_cnt > (ug_proc->file_header.bin_len - RT_IMG_WR_UNIT)) \
                    && (write_len >= (ug_proc->file_header.bin_len -
                                      ug_proc->recv_data_cnt))) {    //last 512 (write directly when get data )
